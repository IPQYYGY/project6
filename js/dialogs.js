		...
		"dialog": function(options) {
			...
			var dialog = {
				...
				render: function() {
					...
					$dom.html([
						...
						'<div class="ps-dialog-footer"></div>'
					].join(''));
					self.$dom = $dom;
					...
					$close.bind({
						...
						'mouseover': function() {
							$(this).addClass('ps-dialog-close-button-hover');
						},
						'mouseout': function() {
							$(this).removeClass('ps-dialog-close-button-hover');
						}
					});
					$(self).bind({
						'close': function() {
						...
						'setTitle': function(e, title) {
							...
							if(typeof title == 'string') {
								$title.find("span").empty().html(title);
							} else {
								$title.find("span").empty().append(title);
							}
						},
						...
						'setCloseIcon': function(e, icon) {
							...
							if(typeof icon == 'string') {
								$close.empty().html(icon);
							} else {
								$content.empty().append(icon);
							}
						}
					});
					if(ops.isDrags) {
						self.dragndrop($dom, $title);
					}
				...
				dragndrop: function(d, tl) {
					...
					var dragAndDrop = {
						'down': function(e) {
							...
							startX = e.pageX;
							startY = e.pageY;
							startLeft = d.offset().left;
							startTop = d.offset().top;
							e.preventDefault();
							tl.focus();
						},
						'drop': function() {
			...
			var handler = function() {
				...
				return {
					...
					dispose: function() {
						$dialog.trigger("dispose");
					},
					setTitle: function(title) {
						$dialog.trigger("setTitle", [title]);
					},
					setContent: function(content) {
						$dialog.trigger("setContent", [content]);
					},
					setFooter: function(footer) {
						$dialog.trigger("setFooter", [footer]);
					},
					setCloseIcon: function(icon) {
						$dialog.trigger("setCloseIcon", [icon]);
					}
				}
			}
			dialog.render();
